  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontFamily: "DM Sans", fontSize: 17, fontWeight: 700, marginBottom: 4 }}>
        {label}{" "}
        {optional && (
          <span style={{ fontSize: 13, fontWeight: 400, color: "#888" }}>(optional)</span>
        )}
      </div>
      ...
      <div
        ...
        style={{
          border: uploaded ? "2px solid #22a355" : "2px dashed #aaa",
          borderRadius: 2,
          padding: "18px 16px",
          textAlign: "center",
          cursor: "pointer",
      ...
      >
        {uploaded ? (
          <span style={{ color: "#22a355", fontFamily: "DM Sans", fontSize: 17 }}>
            ✓ {uploaded.name}
          </span>
        ) : (
          <>
            <div style={{ fontSize: 22, marginBottom: 4 }}>📎</div>
            <div style={{ fontFamily: "DM Sans", fontSize: 16, color: "#555" }}>
...
export default function Step3({ onNext, onBack, companyName }: Step3Props) {
  ...
  const setW9 = (k: string) => (v: string) =>
    setW9Form((f) => ({ ...f, [k]: v }));

  const w9Complete =
    uploads["w9"] || (w9Mode === "fill" && w9Form.name && w9Form.ssn);
  ...
  const sendAgentEmail = async () => {
    ...
    try {
      ...
      // still show as sent — email may work on server
      setEmailSent(true);
    } finally {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: "32px 20px" }}>
      ...
      <h2 style={{ fontFamily: "DM Sans", fontSize: 34, fontWeight: 700, marginBottom: 4 }}>
        Documents & W-9
      </h2>
      <p style={{ color: "#666", fontSize: 15, marginBottom: 24 }}>
        Upload your documents. W-9 and COI are required to get set up.
      </p>
      ...
      <Box style={{ padding: 24, marginBottom: 16 }}>
          ...
          {(
            ...
            <button
              ...
              style={{
                flex: 1,
                padding: "10px 8px",
                border: "2px solid " + (w9Mode === m ? RED : "#ccc"),
                background: w9Mode === m ? "#fff5f5" : "white",
                fontFamily: "DM Sans",
                fontSize: 16,
                fontWeight: 700,
                color: w9Mode === m ? RED : "#666",
                borderRadius: 2,
                cursor: "pointer",
              }}
            >
        ...
        {w9Mode === "upload" ? (
          <UploadBox
            label="W-9 Form"
            hint="Signed W-9 tax form"
            optional={false}
            uploaded={uploads["w9"]}
            onUpload={(f) => setUpload("w9", f)}
          />
        ) : (
          <div style={{ border: "1.5px dashed #ccc", borderRadius: 2, padding: 20, background: "#fafaf8" }}>
            <div style={{ fontFamily: "DM Sans", fontSize: 16, color: "#666", marginBottom: 14 }}>
              W-9 Online Form
            </div>
            ...
            <div style={{ fontSize: 11, color: "#aaa", marginTop: 8 }}>
              Full W-9 will be generated as a PDF upon submission.
            </div>
          </div>
        )}
      </Box>
      ...
      <Box style={{ padding: 24, marginBottom: 24 }}>
        ...
        <UploadBox
          ...
          hint="COI naming Simon Express as certificate holder — PO Box 1582, Riverton, UT 84065"
          optional={false}
          uploaded={uploads["ins"]}
          onUpload={(f) => setUpload("ins", f)}
        />
        ...
        <div style={{ marginBottom: 20, padding: 16, background: "#f8f8f6", border: "1.5px dashed #ccc", borderRadius: 2 }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>
            Have your agent send the COI directly
          </div>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 10 }}>
            Enter your insurance agent&apos;s email and we&apos;ll send them a request automatically, CC&apos;d to dispatch@simonexpress.com.
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              value={agentEmail}
              onChange={(e) => { setAgentEmail(e.target.value); setEmailSent(false); }}
            ...
            />
            <Btn
              onClick={sendAgentEmail}
              disabled={!agentEmail || emailSending}
              variant="secondary"
          ...
          {emailSent && (
            <div style={{ marginTop: 10, padding: "10px 14px", background: "#edfaf3", border: "1.5px solid #22a355", borderRadius: 2, fontSize: 13 }}>
              <strong style={{ color: "#22a355" }}>✓ Email sent to {agentEmail}</strong>
              <br />
              <span style={{ color: "#555" }}>
                <strong>CC:</strong> dispatch@simonexpress.com<br />
                <strong>Request:</strong> COI naming Simon Express, PO Box 1582, Riverton, UT 84065 as certificate holder.
              </span>
            </div>
          )}
        </div>
        ...
        {[
          ...
          ["factoring", "Factoring Letter", "If you use a factoring company"],
      ...
      {(!w9Complete || !coiComplete) && (
        <div style={{ marginBottom: 16, padding: "12px 16px", background: "#fff5f5", border: "1.5px solid #ffaaaa", borderRadius: 2, fontSize: 13, color: RED, fontWeight: 600, lineHeight: 1.7 }}>
          ...
          {!coiComplete && <div>⚠ Certificate of Insurance is required — upload a COI or send a request to your insurance agent above.</div>}
        </div>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <Btn
          onClick={() => onNext({ uploads: Object.fromEntries(Object.entries(uploads).map(([k, v]) => [k, v?.name])), w9Mode, w9Form, agentEmail, emailSent })}
